...
#########################################################################
# 
# $Header: /opt/cvs/python/packages/share1.5/Pmv/grid3DCommands.py,v 1.76 2008/05/16 18:26:57 sargis Exp $
# $Id: grid3DCommands.py,v 1.76 2008/05/16 18:26:57 sargis Exp $
"""This module integrates Volume.Grid3D with PMV for rendering 3D Grids.
...
Grid_ICONPATH = findFilePath('Icons', 'Pmv')
   ...
   \nSynopsis:\n
    ...
    def doit(self, gridFile, name=None, show=True, normalize=True):
        ...
        if grid3D:
            ...
            if self.vf.grids3D.has_key(grid_basename):
                grid_basename += "_"+str(self.grid_counter) 
            def returnStringRepr():
                return None, "\"" + grid_basename + "\""
            grid3D.returnStringRepr = returnStringRepr
            if not hasattr(grid3D,'geomContainer'):
                grid3D.geomContainer = {}
                g = Geom(grid_basename)
            ...
            self.vf.grids3D[grid_basename] = grid3D
            
            if self.vf.Grid3DCommands.root:
                grid_name = grid_basename
    ...
    def askMapType(self):
        ...
        elif choice=='AVS/FLD Binarcmd.y':self.reader=ReadFLDBinary()
        else: 
            tkMessageBox.showerror("Error: in choosing a map", 
...
class Grid3DCommands(MVCommand):
...
class IGraph(Tkinter.Frame):
    ...
    def __init__(self, master, width = width, height = height, 
        ...
        self.menu.add_command(label='Color...',command=self.colorBar) 
    ...
    def plot_historgam(self, event = None, data = None):  
        if data is not None:
            self.data = data
    ...
    def setMin(self, text):
        ...
        hist = numpy.histogram(self.cmd.grid.data.flat,bins=self.width+100, range=bound)
...
class IsocontourCommand(MVCommand):
...
If None given name = "Grid3D_"+str(self.grid_counter)\n   
...
If None given name = "Grid3D_"+str(self.grid_counter)\n   
    ...
    def select(self):
        ...
        if not hasattr(grid,'hist'):
            ...
            hist = numpy.histogram(grid.data.flat,bins=entry['widget'].width+100)
            grid.hist = hist
        ...
        entry['widget'].plot_historgam(data=self.grid.hist)
...
class OrthoSliceCommand(MVCommand):
    def __init__(self, func=None):
        MVCommand.__init__(self)
...
#                                   "Please add and select grid first.", parent=parent)
    ...
    def createX(self):
        ...
        if self.X_vis.get():
            ...
            self.grid._X_Vis = True
            geom.Set(vertices=vertices, array=data)
            if self.grid.path.endswith('.map'):
                if self.grid.mini < 0:
...
class myTF(TableManager):
    ...
    def __init__(self, master, xminval=0, xmaxval=255,
        ...
        self.colorMapB = Tkinter.Button(self.master,text='Colormap',command=self.colorGUI)
        self.colorMapB.pack()
        self.balloon.bind(self.colorMapB, "Colormap GUI uploads Colormap without using above Transfer Function editor.")
    ...
    def colorGUI(self):
        ...
        if hasattr(self,'ColorMapGUI'):
            ...
            self.colorMapB.configure(state='disabled')
            return
        ramp = RGBARamp()
        ...
        def dismissCmap():
            self.colorMapB.configure(state='normal')
            if self.ColorMapGUI.master.winfo_ismapped():
                self.ColorMapGUI.master.withdraw()

        self.ColorMapGUI.dismiss.configure(command = dismissCmap)
        self.ColorMapGUI.master.protocol('WM_DELETE_WINDOW', dismissCmap)
    ...
    def setMin(self, text):
        ...
        datamap['src_max'] = self.cmd.grid.hist[1][-1]
        datamap['dst_min'] = 0
        datamap['dst_max'] = 255
        datamap['map_type'] = 'linear'
        mapper = MapGridData()
        ...
        hist = numpy.histogram(self.cmd.grid.data.flat,bins=self.lut_list[0].width+100, range=bound)
        self.cmd.grid.hist = hist
        self.data = hist
        self.plot_historgam()
    ...
    def setMax(self, text):
        ...
        bound = [self.cmd.grid.hist[1][0],hMax]
        datamap = {}
        datamap['src_min'] = self.cmd.grid.hist[1][0]
    ...
    def plot_historgam(self, event=None):

        grid = self.cmd.grid            

        if not hasattr(grid,'hist'):
            hist = numpy.histogram(grid.data.flat,bins=self.lut_list[0].width+100)
            grid.hist = hist                    

        self.lut_list[0].min_x = grid.hist[1][0]
        self.lut_list[0].max_x = grid.hist[1][-1]     
        self.maxEntry.setentry((self.labelsBarFormat)%grid.hist[1][-1])
        self.minEntry.setentry((self.labelsBarFormat)%grid.hist[1][0])

        self.lut_list[0].plot_historgam(data=grid.hist)            
    ...
    def select(self):
        ...
        if not hasattr(grid,'hist'):            
            ...
            if grid_name.endswith('.map'):
                if grid.mini < 0:
                    bound = [grid.mini,-grid.mini]
            hist = numpy.histogram(grid.data.flat,bins=widget.lut_list[0].width+100)
            grid.hist = hist

        #widget.lut_list[0].min_x = grid.hist.min
        #widget.lut_list[0].max_x = grid.hist.max        
        widget.plot_historgam()            
