...
def numericalVolume():
    ...
    assert len(mol)==1
    mol = mol[0]
    num = len(mol.geomContainer.msmsSel.keys())
    self.msmsSel(self.getSelection(), 1.5, 3.0, "mysurf%d"%num)
    msmssrfhandle= mol.geomContainer.msmsSel["mysurf%d"%num][0]
