...
#
# $Header: /opt/cvs/python/packages/share1.5/Pmv/displayCommands.py,v 1.177 2008/02/14 20:52:17 sargis Exp $
# 
# $Id: displayCommands.py,v 1.177 2008/02/14 20:52:17 sargis Exp $
#
...
from ViewerFramework.VF import DeleteAtomsEvent, AddAtomsEvent, EditAtomsEvent
...
##                 self.addUndoCall( (set,), kw, self.name )
    ...
    def doit(self, nodes, lineWidth=2, displayBO=False , only=False,
        ...
        def drawAtoms(mol, atm, displayBO, lineWidth, only, negate):
            ...
            # don't need to do all the sets operation which are inefficient.
            if len(atm) == len(mol.allAtoms):
                ...
                if negate: #if negate, remove current atms from displayed set
                    set = set - atm
            ...
            if len(set)==0:
                ...
                gatoms['nobnds'] = set
                return
            # This is done only if set contains some atoms.
            bonds, atnobnd = set.bonds

            # 1st lines need to store the whole set in the
            ...
            if len(bonds) == 0:
                ...
                if len(indices)==0:
                    ...
                    colors = map(lambda x: x.colors['lines'], set)
            ...
            if len(atnobnd)==0:
                ...
                if len(ggeoms['nobnds'].vertexSet) != 0  and len(ggeoms['nobnds'].vertexSet) != len(vertices):
                #this is needed to fix bug #864
                    redo = False
                else:
                    redo = True
        ...
        for mol, atms, in map(None, molecules, atomSets):
            ...
            drawAtoms(mol, atms, displayBO, lineWidth, only, negate)
    ...
    def onRemoveObjectFromViewer(self, mol):
        ...
        in this command : Here remove bbDisconnactedAfter created for each
        ...
        for l in levels:
            try:
                ...
                print "exception displayCommands line 547"
        #del levelNodes
...
class DisplayCPK(DisplayCommand):
    ...
    def onAddObjectToViewer(self, obj):
        ...
        geomC = obj.geomContainer
        # CPK spheres
        g = Spheres( "cpk", quality = 5 ,visible=0, protected=True)
    ...
    def setupUndoBefore(self, nodes, only=False, negate=False, scaleFactor=1.0,
                        cpkRad=0.0, quality=0, byproperty = False, propertyName = None, propertyLevel = 'Molecule'):
        ...
        bp = defaultValues['byproperty']
        if bp:
            kw['propertyName'] = defaultValues['propertyName']
            kw['propertyLevel'] = defaultValues['propertyLevel']
            kw['byproperty'] = bp
        kw['redraw'] = True
        #print "kw:", kw
        geomSet = []
    ...
    def doit(self, nodes, only=False, negate=False, 
             ...
             propertyLevel='Molecule'):

        ##############################################################
        def drawAtoms( mol, atm, only, negate,
                       scaleFactor, cpkRad, quality, propertyName = None, propertyLevel="Molecule"):

            atm.cpkScale = scaleFactor
            atm.cpkRad = cpkRad
            set = mol.geomContainer.atoms['cpk']
            if len(atm) == len(mol.allAtoms):
                if negate: set = AtomSet()
                else: set = atm
            else:
                ##if negate, remove current atms from displayed set
            ...
            mol.geomContainer.atoms['cpk']=set
        ...
        if quality < 3:
            nodeAtoms = nodes.findType(Atom)
            if len(nodeAtoms) < 500:
                quality = 20
            elif len(nodeAtoms) < 5000:
                quality = 15
            elif len(nodeAtoms) < 10000:
                quality = 10
            else:
                quality = 5
    ...
    def buildFormDescr(self, formName='default'):
        ...
        if formName == 'default':
            ...
            idf.append({'name':'quality',
                        'parent':'radiiGroup',
                        'widgetType':ThumbWheel,
                        'tooltip':'if quality < 3 a default value will be determined based on the number of atoms involved in the command',
                        'wcfg':{ 'labCfg':{'text':'Sphere Quality:', 
                                           'font':(ensureFontCase('helvetica'),12,'bold')},
                                 'showLabel':1, 'width':100,
                                 'min':0, 'max':100, 'type':int, 'precision':1,
    ...
    def guiCallback(self):
        ...
        if val:
            ...
            if not bp:
                ...
                kw['propertyName'] = val['properties'][0]
        ...
        if val:
            ...
            if not form.descr.entryByName.get('propsGroup'):
                ...
                descr.append({'widgetType':Pmw.RadioSelect,
                              ...
                              'defaultValue':self.level,'listoption':self.leveloption,
                ...
                descr.append({'name':'quality1',
                        ...
                        'wcfg':{ 'labCfg':{'text':'Sphere Quality:', 
                                 ...
                                 'showLabel':1, 'width':100,
                                 'min':0, 'max':100, 'type':int, 'precision':1,
                                 'value': defaultValues.get('quality', 0),
                                 'continuous':1,
                                 'oneTurn':10, 'wheelPad':2, 'height':20},
                        'gridcfg':{'sticky':'e'}})
...
current selection cannot be colored using this function."
    ...
    def onAddObjectToViewer(self, obj):
        ...
        geomC = obj.geomContainer
        # CPK spheres
        g = Spheres( "cpkByProp", quality = 5 ,visible=0, protected=True)
        geomC.addGeom(g, parent=geomC.masterGeom, redo=0)
        self.managedGeometries.append(g)
    ...
    def doit(self, nodes, only=False, negate=False, 
        ...
        def drawAtoms( mol, atm, only, negate, 
            ...
            #atm.cpkScale = scaleFactor
            #atm.cpkRad = cpkRad
            set = mol.geomContainer.atoms['cpkByProp']
            if len(atm) == len(mol.allAtoms):
                if negate: set = AtomSet()
                else: set = atm
            else:
            ...
            mol.geomContainer.atoms['cpkByProp']=set
            ...
            if len(set)==0: 
                ...
                set.sort()   # EXPENSIVE...
                colors = map(lambda x: x.colors['cpkByProp'], set)
                #cpkSF = Numeric.array(set.cpkScale)
...
##         molecules, atomSets = self.getNodes(nodes)
    ...
    def doit(self, nodes, only=False, negate=False, bRad=0.3,
        ...
        def drawBalls(mol, atm, only, noBalls, negate, bRad, bScale,
            ...
            set = mol.geomContainer.atoms['balls']
            ## case noballs:
            if noBalls:
                if only:
                    if len(atm) == len(mol.allAtoms):
                    ...
                    mol.geomContainer.geoms['balls'].Set(visible=0,
                                                         tagModified=False)
                    mol.geomContainer.atoms['balls'] = set
                    return
                else:
                    negate = True
            ...
            mol.geomContainer.atoms['balls']=set
            #set.sort()
            bScale = Numeric.array(set.ballScale)
            bRad = Numeric.array(set.ballRad)
            aRad = Numeric.array(set.radius)
            
            ballRadii = bRad + bScale * aRad
            ballRadii = ballRadii.tolist()
            
            b = mol.geomContainer.geoms['balls']
            bcolors = map(lambda x: x.colors['balls'], set)
            b.Set(vertices=set.coords, radii=ballRadii,
                  materials=bcolors,
        ...
        def drawSticks(mol, atm, only, negate, cradius, cquality):
            ...
            if len(set) == 0:
                mol.geomContainer.geoms['sticks'].Set(visible=0,
                                                      tagModified=False)
                mol.geomContainer.atoms['sticks']= set
                return
            ...
            if len(indices) == 0:
                ...
                cRad = set.cRad
                scolors = map(lambda x: x.colors['sticks'], set)
                g.Set( vertices=set.coords, faces=indices, radii=cRad,
...
##                                      tagModified=False)
        ...
        for mol, atm in map(None, molecules, atomSets):
             try:
                 if sticksBallsLicorice == 'Licorice':
                     drawBalls(mol, atm, only, 0, negate, cradius, 0.0,
                           cquality)
                 elif sticksBallsLicorice == 'Sticks only':
                     drawBalls(mol, atm, only, 1, negate, bRad, bScale,
                           bquality)
                 else:
                     drawBalls(mol, atm, only, 0, negate, bRad, bScale,
                           bquality)
                 
                 drawSticks(mol, atm, only, negate, cradius, cquality)
             except:
        ...
        if only and len(self.vf.Mols) != len(molecules):
            ...
            for mol in mols:
                ...
                only=0
                if sticksBallsLicorice == 'Licorice':
                    drawBalls(mol, mol.allAtoms, only, 0, negate, cradius,
                          0, cquality)
                elif sticksBallsLicorice == 'Sticks only':
                    drawBalls(mol, mol.allAtoms, only, 1, negate, bRad,
                          bScale, bquality)
                else:
                    drawBalls(mol, mol.allAtoms, only, 0, negate, bRad,
                          bScale, bquality)
                drawSticks(mol, mol.allAtoms, only, negate, cradius, cquality)
...
"""
    ...
    def doit(self, nodes, only=False, negate=False, bRad=0.3,
        ...
        def drawCABalls(mol, atm, only, noBalls, negate, bRad, bScale,
            ...
            set = mol.geomContainer.atoms['CAballs']
            ## case noballs:
            if noBalls:
                if only:
                    if len(atm) == len(mol.allAtoms):
                    ...
                    mol.geomContainer.geoms['CAballs'].Set(visible=0,
                                                         tagModified=False)
                    mol.geomContainer.atoms['CAballs'] = set
                    return
                else:
                    negate = True
            ...
            bcolors = map(lambda x: x.colors['CAballs'], set)
            b.Set(vertices=set.coords, radii=ballRadii,
                  materials=bcolors,
                  visible=1, tagModified=False)
            if bquality:
        ...
        def drawCASticks(mol, atm, only, negate, cradius, cquality):
                ...
                atm.sort() 
                atm.cRad = cradius
                set = mol.geomContainer.atoms['CAsticks']
                ...
                if len(set) == 0:
                    mol.geomContainer.geoms['CAsticks'].Set(visible=0,
                                                      tagModified=False)
                    mol.geomContainer.atoms['CAsticks']= set
                    return
                ...
                for ch in range(0,len(mol.chains)):    
                    ...
                    if len(set)!=0:
                         set.sort()
                         mygap =FindGap(mats,mol.chains[ch],set)
                ...
                if len(indices) == 0:
                    ...
                    cRad = set.cRad
                    scolors = map(lambda x: x.colors['CAsticks'], set)
                    g.Set( vertices=set.coords, faces=indices, radii=cRad,
                       materials=scolors, visible=1, quality=cquality,tagModified=False)
    ...
    def doit(self, nodes, geomNames=None, only=0, negate=0, nbVert=3):
        ...
        for mol, atm  in map(None,molecules,atomSets):
            ...
            for obj in geomNames:
                ...
                set = mol.geomContainer.atoms[oname]
                ...
                if negate: set = set - atm
                    ...
                    if only: set = atm
                        ...
                        set = atm.union(set)
                mol.geomContainer.atoms[oname]=set
