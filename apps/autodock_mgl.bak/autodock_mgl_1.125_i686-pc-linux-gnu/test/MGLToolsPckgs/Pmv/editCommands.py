...
# $Header: /opt/cvs/python/packages/share1.5/Pmv/editCommands.py,v 1.117 2007/09/12 20:33:59 sargis Exp $
#
# $Id: editCommands.py,v 1.117 2007/09/12 20:33:59 sargis Exp $
#
...
##                 # need to set the nodeLogString of the command that will
##                 # be logged and it is not the GUICommand.
##                 self.vf.add_h.nodeLogString = "self.getSelection()"
...
\nnodes --- TreeNodeSet holding the current selection"""
    ...
    def cont_cb(self, sel):
        ...
        self.form.destroy()
##         if self.vf.userpref['expandNodeLogString']['value'] == 0:
##             self.vf.mergeNPHS.nodeLogString = "self.getSelection()"
...
\nnodes --- TreeNodeSet holding the current selection"""
    ...
    def doit(self, nodes, **kw):
        ...
        for i in range(len(molecules)):
            mol = molecules[i]                
            npHSet = mol.mergeNPH(atomSets[i])
            self.vf.allAtoms = self.vf.allAtoms - npHSet
            if len(npHSet):
                returnVal.append(mol)
            if len(mol.chains.residues) == 1:
                mol.geomContainer.atoms['bonded'] += npHSet
            event = DeleteAtomsEvent(objects=npHSet)
            self.vf.dispatchEvent(event)
...
for each atom and entering each atom's charge in its _charges dictionary.
    ...
    def guiCallback(self):
        ...
        if len(sel):
##             if self.vf.userpref['expandNodeLogString']['value'] == 0:
##                 self.vf.computeGasteiger.nodeLogString = "self.getSelection()"
...
class AverageChargeErrorCommand(MVCommand):
    ...
    def guiCallback(self):
        sel = self.vf.getSelection()
##         if self.vf.userpref['expandNodeLogString']['value'] == 0:
##             self.vf.averageChargeError.nodeLogString = "self.getSelection()"
...
class CheckChargesGUICommand(MVCommand):
    ...
    def guiCallback(self):
        sel = self.vf.getSelection()
##         if self.vf.userpref['expandNodeLogString']['value'] == 0:
##             self.vf.checkResCharges.nodeLogString = "self.getSelection()"
