...
#$Header: /opt/cvs/python/packages/share1.5/Pmv/APBSCommands.py,v 1.154 2008/04/25 00:38:45 vareille Exp $
#
#$Id: APBSCommands.py,v 1.154 2008/04/25 00:38:45 vareille Exp $
...
append(Pmv.APBSCommands.Ion("+ion.toString()+"))")
    ...
    def getCoords(self):
        ...
        molName=os.path.splitext(self.params.molecule1Path)[0].split("/")[-1]
        mol = self.vf.getMolFromName(molName)
        coords = mol.findType(Atom).coords
        if self.params.calculationType == 'Binding energy':
            molName=os.path.splitext(self.params.molecule2Path)[0].split("/")[-1]
            mol = self.vf.getMolFromName(molName)
            if mol:
                coords += mol.findType(Atom).coords
            molName=os.path.splitext(self.params.complexPath)[0].split("/")[-1]
            mol = self.vf.getMolFromName(molName)
            if mol:
                coords += mol.findType(Atom).coords
        return coords
            ...
            if not os.path.exists(filename) or \
              ...
              # Exe_String = sys.executable + \
               ...
               #                                         "\""+full_filename+"\""
                ...
                import subprocess
                returncode = subprocess.call(sys.argv,env={'PYTHONPATH':os.path.split(self.params.pdb2pqr_Path)[0]})
                if returncode:
...
#        self.vf.GUI.MESSAGE_BOX.tx.component('text').config(cursor='xterm')   
    ...
    def apbsOutput(self, molecule1=None, molecule2=None, _complex=None, blocking=False ):
        ...
        if molecule1:
            ...
            self.vf.APBSRun(molecule1, molecule2, _complex,
    ...
    def saveProfile(self, Profilename="Default", fileFlag=False, flagCommand=False):
        ...
        if fileFlag:
            ...
            if os.path.exists(potential_dx):
                ...
                shutil.copy(potential_dx,dest_path)          
    ...
    def loadProfile(self, filename = None):
        ...
        if filename:
            ...
            if os.path.exists(file_potential): 
                shutil.copy(file_potential,os.path.join(self.params.\
...
#                           configure(text = 'Remote APBS calculation is done')
    ...
    def Download(self):
        ...
        if tmp:
            ...
            f.descr.entryByName['APBSservicesLabel5']['widget'].\
                                                            configure(text = '')
            return     
...
#                    })
            ...
            if APBSservicesFound:
                ...
                ifd.append({'widgetType':Pmw.ComboBox,
                            ...
                            'parent':'APBSservicesGroup',
                            'wcfg':{'scrolledlist_items':
                                       ('http://ws.nbcr.net/opal/services/ApbsOpalService',),
...
#                    })
                ...
                ifd.append({'name':'APBSservicesLabel5',
                                    'widgetType':Tkinter.Label,
                                    'parent':'APBSservicesGroup',
                                    'wcfg':{'text':'This feature is in alpha test stage',
                                            'fg':'red'},
                                 'gridcfg':{'sticky':'ensw', 'row':11,'column':0,
                                                'columnspan':2}
                                    })
            else:
    ...
    def __call__(self,  molecule1=None, molecule2=None, _complex=None,
        ...
        """None <--- APBSRun(nodes, **kw)\n
            \nOptional Arguments :\n
              ...
              APBSParamName = Name of the key in mol.APBSParams dictionary
        """             
        if molecule1 is None and len(self.vf.Mols) == 1:
