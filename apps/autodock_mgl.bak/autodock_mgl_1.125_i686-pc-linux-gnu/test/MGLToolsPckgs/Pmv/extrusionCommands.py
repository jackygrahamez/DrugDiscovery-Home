...
# $Header: /opt/cvs/python/packages/share1.5/Pmv/extrusionCommands.py,v 1.40 2007/07/24 17:30:38 vareille Exp $
#
# $Id: extrusionCommands.py,v 1.40 2007/07/24 17:30:38 vareille Exp $
#
...
class ComputeSheet2DCommand(MVCommand):
    ...
    def doit(self, nodes, sheet2DName, ctlAtmName, torsAtmName,
        ...
        for chain in chains:
            ...
            sheet2DRes, sheetCoords, inHelix = self.getSheet2DRes(chain,
                                                                  ctlAtmName,
                                                                  torsAtmName,
                                                                  buildIsHelix)
...
class Nucleic_Acids_properties(MVCommand):
    ...
    def __call__(self, color_A =  [1,0,0], color_G = [0,0,1], color_T = [0,1,0],
        ...
        apply(self.doitWrapper, (color_A, color_G, color_T, color_C, color_U,
                                 height_purine, height_pyrimidine, 
                                 scale_purine, scale_pyrimidine), kw)
    ...
    def buildFormDescr(self, formName):
        if formName == 'Display':
            idf = InputFormDescr(title =
                                  "Set Nucleic Acids colors and scaling factor")
            ...
            idf.append( {'name': 'height_purine',
                         ...
                         'parent':'Purines',                                                  
                         'wcfg':{'label': "Height",
                               'minval':0.01,'maxval':1, 'incr': 0.01, 'init':0.4,
                                 'labelsCursorFormat':'%1.2f', 'sliderType':'float',
                                 'entrywcfg':{'width':4},
                                 'entrypackcfg':{'side':'right'}},
            ...
            idf.append( {'name': 'scale_purine',
                         ...
                         'parent':'Purines',                                                  
                         'wcfg':{'label': " Scale",
                               'minval':0.5,'maxval':2, 'incr': 0.01, 'init':1.3,
                                 'labelsCursorFormat':'%1.2f', 'sliderType':'float',
                                 'entrywcfg':{'width':4},
                                 'entrypackcfg':{'side':'right'}},
    ...
    def guiCallback(self):
        ...
        if val:
            ...
            scale_purine = self.idf.entryByName['scale_purine']['widget'].get()
            scale_pyrimidine = self.idf.entryByName['scale_pyrimidine']\
                                                                ['widget'].get()
            height_purine = self.idf.entryByName['height_purine']['widget'].get()
            height_pyrimidine = self.idf.entryByName['height_pyrimidine']\
                                                                ['widget'].get()
