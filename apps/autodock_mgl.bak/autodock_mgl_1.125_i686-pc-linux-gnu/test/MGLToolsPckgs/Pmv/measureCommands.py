...
#
# $Header: /opt/cvs/python/packages/share1.5/Pmv/measureCommands.py,v 1.43 2007/07/24 17:30:38 vareille Exp $
#
# $Id: measureCommands.py,v 1.43 2007/07/24 17:30:38 vareille Exp $
#
...
class MeasureDistanceGUICommand(MeasureGUICommand):
    """Accumulates picked atoms. Draws lines between pairs of selected atoms, labeled by distance (color-coded yellow).Userpref 'measureDistanceSL' sets the 'snakeLength' which is how many distance measures can be displayed at the same time.When more than that number are measured, the first distance measured is no longer labeled.
   \nPackage : Pmv
   \nModule  : measureCommands
   ...
   \nRequired Argument:\n        
    ...
    def guiCallback(self, event=None):
        ...
        self.vf.setICOM(self.vf.measureDistanceCtrl, modifier='Control_L', topCommand=0)

#    def stopICOM(self):
#        MeasureGUICommand.stopICOM(self)
#        self.vf.measureDistanceCtrl.spheres.Set(vertices=[])
#        self.measureDistanceCtrl.lines.Set(vertices=[], faces=[])
#        self.measureDistanceCtrl.labels.Set(vertices=[])
    ...
    def onAddCmdToViewer(self):
        ...
        if not self.vf.commands.has_key('measureDistanceCntr'):
            self.vf.loadCommand('measureCommands', 'measureDistanceCtrl',
                                'Pmv', topCommand=0) 
...
Valid values are integers>0"""
        self.vf.userpref.add('measureDistanceSL', 4,
...
turned off  and viewer's current object is not Root."""
        choices = ['yes','no']
        self.vf.userpref.add('continuousUpdateDist', 'yes', choices,
...
MeasureDistanceGUICommandGUI.addMenuCommand('menuRoot', 'Display', 'Distance',
                                                        cascadeName = 'Measure')

class MeasureDistanceCtrlGUICommand(MeasureGUICommand):
    """Accumulates picked atoms. Draws lines between pairs of selected atoms, labeled by distance (color-coded yellow).Userpref 'measureDistanceSL' sets the 'snakeLength' which is how many distance measures can be displayed at the same time.When more than that number are measured, the first distance measured is no longer labeled.
   \nPackage : Pmv
   \nModule  : measureCommands
   \nClass   : MeasureDistanceGUICommand
   ...
   \nRequired Argument:\n        
    ...
    def onAddCmdToViewer(self):
        ...
        miscGeom = self.vf.GUI.miscGeom
        measure_geoms = check_measure_geoms(self.vf.GUI)
        self.masterGeom = Geom('measureDistGeomCtrl', shape=(0,0), 
                                pickable=0 )
        self.masterGeom.isScalable = 0
        self.vf.GUI.VIEWER.AddObject(self.masterGeom, parent = measure_geoms)
        self.lines = IndexedPolylines('distLine', materials = ((1,1,0),),
...
class MeasureAngleGUICommand(MeasureGUICommand):
   ...
   \nRequired Argument:\n        
    ...
    def onAddCmdToViewer(self):
        ...
        self.labels = GlfLabels(name='angleLabel', shape=(0,3),
...
Valid values are integers>0"""
        self.vf.userpref.add('measureAngleSL', 4,
                             callbackFunc = [self.setLength_cb],
                             validateFunc = lambda x: x>0, doc=doc)
        #used after startICOM is invoked
...
turned off  and viewer's current object is not Root."""
        choices = ['yes','no']
        self.vf.userpref.add('continuousUpdateAngle', 'yes', choices,
                             callbackFunc = [self.continuousUpdate_cb],
                             doc=doc )
        self.snakeLength = 4
...
MeasureAngleGUICommandGUI.addMenuCommand('menuRoot', 'Display', 'Angle',
                                                        cascadeName = 'Measure')
...
class MeasureTorsionGUICommand(MeasureGUICommand):
   ...
   \nRequired Argument:\n        
    ...
    def onAddCmdToViewer(self):
        ...
        self.masterGeom.isScalable = 0
        self.vf.GUI.VIEWER.AddObject(self.masterGeom, parent = measure_geoms)
        self.lines = IndexedPolygons('torsionLine', materials = ((0,1,1),),
                                     culling=GL.GL_NONE,inheritStipplePolygons=0,
                                     inheritMaterial=0, stipplePolygons=1,
                                     backPolyMode=GL.GL_FILL,
                                     frontPolyMode=GL.GL_FILL, protected=True,)
        ...
        self.labels = GlfLabels(name='torsionLabel', shape=(0,3),
...
Valid values are integers>0"""
        self.vf.userpref.add('measureTorsionSL', 4,
                             callbackFunc=[self.setLength_cb],
                             validateFunc = lambda x: x>0, doc=doc)
        #used after startICOM is invoked
...
turned off  and viewer's current object is not Root."""
        choices = ['yes','no']
        self.vf.userpref.add('continuousUpdateTorsion', 'yes', choices,
                             callbackFunc = [self.continuousUpdate_cb],
                             doc=doc )
        self.snakeLength = 4
...
MeasureTorsionGUICommandGUI.addMenuCommand('menuRoot', 'Display', 'Torsion',
                                            cascadeName = 'Measure')
...
commandList = [
        ...
        {'name':'measureDistanceCtrl', 'cmd':MeasureDistanceCtrlGUICommand(),
                'gui':None}, 
