...
# $Header: /opt/cvs/python/packages/share1.5/Pmv/selectionCommands.py,v 1.129 2008/01/25 19:57:49 sargis Exp $
#
# $Id: selectionCommands.py,v 1.129 2008/01/25 19:57:49 sargis Exp $
#
            ...
            for lGeom in mol.geomContainer.geoms.values():
                if isinstance(lGeom, IndexedPolygons) and lGeom.parent.name == 'secondarystructure':
                    ...
                    for selectedResidue in selectedResidueSet:
                        if lGeom.resfacesDict.has_key(selectedResidue):
...
##                                        self.vf.ICmdCaller.level.value.__name__)
    ...
    def updateSelectionIcons(self):
        ...
        if self.vf.userpref['showSelectionSpheres']['value']==1:
...
mvSaveSetCommandGuiDescr = {'widgetType':'Menu', 'menuBarName':'menuRoot',
                            ...
                            'menuEntryLabel':'Save current selection as a set'}
...
mvSelectSetCommandGuiDescr = {'widgetType':'Menu', 'menuBarName':'menuRoot',
                              'menuButtonName':'Select',
                              'menuEntryLabel':'Select a set'}

MVSelectSetCommandGUI = CommandGUI()
MVSelectSetCommandGUI.addMenuCommand('menuRoot', 'Select', 'Select a set',
                                     separatorBelow=1 )
...
class MVSelectFromStringCommand(MVCommand):
    ...
    def doit(self, mols='', chains='', res='', atoms='', negate=False,
        ...
        userpref = self.vf.userpref['selectStringMatchMode']['value']
        caseSensitive = True
...
#                    })
        ...
        ifd.append({'name': 'Save Selection',
                    'widgetType':Tkinter.Button,
                    'wcfg':{'text': 'Save Selection As A Set',
        ...
        #ifd.append({'name': 'Invert Selection',
        #    'widgetType':Tkinter.Button,
        #    'wcfg':{'text': 'Invert Selection',
        #    'borderwidth':1 },
        #    'gridcfg':{'sticky':Tkinter.W+Tkinter.E,'row':-1,'column':3,
        #         'columnspan':3},
        #    'command': self.invertSel})
        #ifd.append({'name': 'Select Pick',
        #    'widgetType':Tkinter.Checkbutton,
        #    'text': 'Add to Selection String by Picking',
        #    'variable': self.selPick,
        #    'gridcfg':{'sticky':Tkinter.W,'row':7,'column':0,
        #         'columnspan':4},
        #    'command': self.startSelPick})
        ifd.append({'name': 'Show SelSpheres',
                    'widgetType':Tkinter.Checkbutton,
                    'wcfg':{ 'text': 'Show Selection Spheres',
    ...
    def guiCallback(self):
        if not hasattr(self, 'ifd'):
            ...
            self.form.deiconify()
            self.form.lift()
        self.showSelSpheres.set(self.vf.userpref['showSelectionSpheres']['value'])
    ...
    def getSetsVal(self, event=None):
        ...
        #these 3 are hard-wired atom sets
        specialCases = ['backbone', 'backbone+h' , 'sidechain']
    ...
    def toggleSelSpheres(self, event = None):
        if self.vf.userpref['showSelectionSpheres']['value']==1:
            self.vf.userpref.set('showSelectionSpheres',0)
        else:
            self.vf.userpref.set('showSelectionSpheres',1)
        self.showSelSpheres.set(self.vf.userpref['showSelectionSpheres']['value'])
...
class MVSelectSphericalRegion(MVCommand):
    ...
    def doit(self, centerList, radius, selList):
        ...
        if hasattr(self.vf, 'logAllFile'):
            self.vf.logAllFile.write("from numpy.oldnumeric import array\n")
        if not centerList:
        ...
        if len(ats)>0:
            ...
            if vflev!=lev:
                self.vf.setSelectionLevel(lev, busyIdle=0, log=0)
            self.vf.clearSelection()
            self.vf.select(atSet, topCommand=0)
        if self.form:
            self.form.lift()
...
class MVInvertSelection(MVCommand):
    """Inverts current selection. If there is None, the user is given 
    ...
    \nSynopsis:\n   
         None <- invertSelection(invertLevel, **kw)
    \nRequired Arguments:\n      
    ...
    def setupUndoBefore(self, invertLevel):
        select = self.vf.select
        oldselection = self.vf.selection
        self.addUndoCall( (), {}, self.vf.clearSelection.name )
        if len(oldselection):
    ...
    def doit(self, invertLevel):
        
        sel = self.vf.selection
        lev = self.vf.selectionLevel
        if len(sel):
            old=sel[:]
        else:
            old = []
    ...
    def __call__(self, invertLevel, **kw):
        """None <--- invertSelection(invertLevel, **kw)
        \ninvertLevel --- 'all' or 'molecule'"""
        
        # assert invertLevel is a good value
        assert invertLevel in ['all', 'molecule']
        apply(self.doitWrapper,(invertLevel,), kw)
