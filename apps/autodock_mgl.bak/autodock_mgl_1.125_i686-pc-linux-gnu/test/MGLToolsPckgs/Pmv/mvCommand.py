...
# $Header: /opt/cvs/python/packages/share1.5/Pmv/mvCommand.py,v 1.43 2008/06/10 22:42:10 vareille Exp $
#
# $Id: mvCommand.py,v 1.43 2008/06/10 22:42:10 vareille Exp $
#
...
from MolKit.molecule import Molecule, Atom, BondSet
    ...
    def execCmd(self, objects):
        ...
        if isinstance(objects, TreeNodeSet) and \
           not isinstance(objects, BondSet) :
            ...
            nodes = objects
        if nodes:
            InteractiveCmdCaller.execCmd(self, nodes)
...
##         a list of python statments called before is also built. This list
        ...
        if not (self.vf.selectionLevel in [Molecule, Protein] and \
           ...
           self.vf.selectionLevel!= Klass:
            if len(self.vf.selection)!=0:
                ...
                str1 = apply( self.logString,
                (self.vf.selectionLevel,),
                              {'KlassSet':None, 'topCommand':0} )
                undoString = str1 + ';'\
                             'self.select('+ \
                             '"' + self.vf.getSelection().full_name() + \
                             '", only=1, topCommand=0 ) '
                self.vf.undo.addEntry(undoString, self.name)
            else:
