...
# $Header: /opt/cvs/python/packages/share1.5/Pmv/extruder.py,v 1.31 2007/10/03 01:05:13 vareille Exp $
#
# $Id: extruder.py,v 1.31 2007/10/03 01:05:13 vareille Exp $
#
...
class Sheet2D:
    ...
    def compute(self, coords , isHelix, nbrib = 2,
        ...
        binormals = self.computeBinormals()
        normals = self.computeNormals()
        self.matrixTransfo= self.buildTransformationMatrix(binormals, normals)
    ...
    def buildTransformationMatrix(self, binormals, normals):
        matrixTransfo = Numeric.zeros( (len(self.path), 3, 3) ).astype('f')
        for i in range(len(self.path)):
            matrixTransfo[i][0] = normals[i]
            matrixTransfo[i][1] = binormals[i]
            matrixTransfo[i][2] = self.path[i]
...
class ExtrudeObject:
    ...
    def __init__(self, path3D, matrix, shape, cap1 = 0, cap2 = 0, arrow = 0,
        ...
        self.shape = shape
        self.arrow = arrow
        if not isinstance(self.shape, Rectangle2D): self.arrow = 0
        self.larrow = larrow
        self.warrow = warrow
        self.cap1 = cap1
        self.cap2 = cap2
...
##         self.indexEnd = self.residuesInChain.index(self.ssElt.end)
    ...
    def getExtrudeResidues(self, resSet):
...
def ExtrudeNA(chain):
    ...
    chain.sheet2D['ssSheet2D'].compute(coord, len(chain.residues)*(False,), 
