...
#########################################################################
#
# $Header: /opt/cvs/python/packages/share1.5/Pmv/labelCommands.py,v 1.64 2007/07/31 00:40:04 vareille Exp $
#
# $Id: labelCommands.py,v 1.64 2007/07/31 00:40:04 vareille Exp $
#
    ...
    def onAddObjectToViewer(self, obj):
        ...
        for i in range(len(classNames)):
            ...
            if not geomC.geoms.has_key(geomName):
                j = i + 1
                g = GlfLabels(geomName,
                      #inheritMaterial=0,
                      fontTranslation=(0,0,.2), # so the label is less hidden by the structure
                      fontScales=(j*.3,j*.3,j*.3), # different size for atom, res, chain, mol
                      visible=0,
                      pickable=0,
                     )
