...
#
#$Header: /opt/cvs/python/packages/share1.5/Pmv/splineCommands.py,v 1.51 2007/12/04 19:25:54 vareille Exp $
#
#$Id: splineCommands.py,v 1.51 2007/12/04 19:25:54 vareille Exp $
#
#
...
class ComputeSplineGUICommand(MVCommand):
    ...
    def onAddCmdToViewer(self):
        ...
        if self.vf .hasGui and \
           not self.vf.commands.has_key('displaySplineAsLine'):
...
class ExtrudeSplineCommand(MVCommand):
    ...
    def onAddCmdToViewer(self):
        if self.vf.hasGui and not self.vf.commands.has_key('computeSplineGC'):
            self.vf.loadCommand("splineCommands", 'computeSplineGC',
                                "Pmv", topCommand = 0)
            
        if self.vf.hasGui and \
           not self.vf.commands.has_key('displayExtrudedSpline'):
...
class CustomSplineCommand(MVCommand):
    ...
    def onAddCmdToViewer(self):
        if self.vf.hasGui and \
           not self.vf.commands.has_key('computeSpline'):
            self.vf.loadCommand("splineCommands",
                                "computeSpline", "Pmv",
                                topCommand = 0)
        if self.vf.hasGui and \
           not self.vf.commands.has_key('extrudeSpline'):
            self.vf.loadCommand("splineCommands",
                                "extrudeSpline", "Pmv",
                                topCommand = 0)
        if self.vf.hasGui and \
           not self.vf.commands.has_key('displaySpline'):
            self.vf.loadCommand("splineCommands",
                                "displayExtrudedSpline", "Pmv",
                                topCommand = 0)
