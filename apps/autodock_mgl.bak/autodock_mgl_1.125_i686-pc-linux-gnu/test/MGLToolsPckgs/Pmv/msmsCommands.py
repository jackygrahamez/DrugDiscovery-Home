...
#
# $Header: /opt/cvs/python/packages/share1.5/Pmv/msmsCommands.py,v 1.124 2008/04/08 21:47:01 vareille Exp $
# 
# $Id: msmsCommands.py,v 1.124 2008/04/08 21:47:01 vareille Exp $
#
...
from Pmv.guiTools import AugmentedMoleculeChooser
...
if hasattr( Pmv, 'numOfSelectedVerticesToSelectTriangle') is False:
    ...
    print "LALALALA"
    Pmv.vf.userpref.add('sharpColorBoundariesForMsms', 'yes', ('yes','no'),
...
class DisplayMSMS(DisplayCommand):
        ...
        for mol, atoms in zip(molecules, atomSets):
            ...
            for name in geomC.msms.keys():
                ...
                for a in atoms:
                    ...
                    coords.append( a.coords+[a.radius] )
    ...
    def doit(self, nodes, surfName='all', negate=False, only=False, nbVert=Pmv.numOfSelectedVerticesToSelectTriangle):
        ...
        for mol, atoms in map(None, molecules, atomSets):
            ...
            for sName in names:
                ...
                if negate: lSet = lSet - atm

                ##if only, replace displayed set with current atms 
                else:
                    if only: lSet = atm
                    else: lSet = atm.union(lSet)
    ...
    def doit(self, nodes, surfName='MSMS-MOL', pRadius=1.5, density=1.0,
	    ...
	    col = geomC.getGeomColor(surfName)
            g.Set( vertices=vf[:,:3], vnormals=vf[:,3:6], faces=f[:,:3],
                   materials=col, tagModified=False )
            if display:
                if not self.vf.commands.has_key('displayMSMS'):
                    self.vf.loadCommand("msmsCommands", ['displayMSMS',],
                                    topCommand=0)
