...
"""
#
#$Header: /opt/cvs/python/packages/share1.5/Pmv/traceCommands.py,v 1.29 2008/06/02 19:56:27 sargis Exp $
#
#$Id: traceCommands.py,v 1.29 2008/06/02 19:56:27 sargis Exp $
#
...
class ComputeTraceCommand(MVCommand):
    ...
    def onAddCmdToViewer(self):
        if self.vf.hasGui and not self.vf.commands.has_key('computeSheet2D'):
...
class ExtrudeTraceCommand(MVCommand):
    ...
    def onAddCmdToViewer(self):
        if self.vf.hasGui and not self.vf.commands.has_key('computeTrace'):
            self.vf.loadCommand("traceCommands", 'computeTrace',
                                "Pmv", topCommand = 0)
            
        if self.vf.hasGui and \
           not self.vf.commands.has_key('displayTrace'):
...
class DisplayTraceCommand(DisplayCommand):
    ...
    def onAddCmdToViewer(self):
        ...
        if self.vf.hasGui and not self.vf.commands.has_key('extrudeTrace'):
...
class CustomTraceCommand(MVCommand):
    ...
    def onAddCmdToViewer(self):
        if self.vf.hasGui and \
           not self.vf.commands.has_key('computeTrace'):
            self.vf.loadCommand("traceCommands",
                                "computeTrace", "Pmv",
                                topCommand = 0)
        if self.vf.hasGui and \
           not self.vf.commands.has_key('extrudeTrace'):
            self.vf.loadCommand("traceCommands",
                                "extrudeTrace", "Pmv",
                                topCommand = 0)
        if self.vf.hasGui and \
