...
class PmvNode(NetworkNode):
    ...
    def __init__(self, vf=None, name='PMV', **kw):
        ...
        code = """def doit(self, cmdName, molecule):
    ...
    if hasattr(self.vf, 'select'):
        ...
        self.outputData(nodes=self.vf.getSelection())
...
"""
    def __init__(self, constrkw = {},  name='Get MSMS Geom', **kw):
        ...
        self.inputPortsDescr.append(name='surfaceName', datatype='string')

        self.outputPortsDescr.append(name='msmsGeom', datatype='geom')

        code = """def doit(self, molecule, surfaceName='MSMS-MOL'):
    if molecule:
...
"""
    ...
    def __init__(self, name='Set Instances', **kw):
        ...
        code = """def doit(self, molecule, matrices=[Numeric.identity(4,'f')]):
    if molecule:
