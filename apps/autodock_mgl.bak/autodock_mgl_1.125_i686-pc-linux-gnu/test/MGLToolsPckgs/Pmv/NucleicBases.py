...
# $Header: /opt/cvs/python/packages/share1.5/Pmv/NucleicBases.py,v 1.11 2007/07/24 17:30:38 vareille Exp $
# $Date: 2007/07/24 17:30:38 $
# $Id: NucleicBases.py,v 1.11 2007/07/24 17:30:38 vareille Exp $
...
def Add_Nucleic_Bases(g, properties = None):
    ...
    for i in range(total_res):
        ...
        O4 = Numeric.array(residues[i].atoms.objectsFromString('O4\*')[0].coords)
        C4 = Numeric.array(residues[i].atoms.objectsFromString('C4\*')[0].coords)
        C1 = Numeric.array(residues[i].atoms.objectsFromString('C1\*')[0].coords)
        C3 = Numeric.array(residues[i].atoms.objectsFromString('C3\*')[0].coords)
        C2 = Numeric.array(residues[i].atoms.objectsFromString('C2\*')[0].coords)
...
#         geom_bases.Set(inheritSharpColorBoundaries=False, sharpColorBoundaries=False,)
