...
$result = mysql_query("SELECT `bt`,`lig`, max(`steps`) as STEPS, AVG(`dist2`)*10 as AVG_DIST, STD(`dist2`)*10 as STD_DIST, (AVG(`dist2`) - STD(`dist2`)*2)*10 as MIN, (AVG(`dist2`) + STD(`dist2`)*2)*10 as MAX FROM `mdrun_progress` GROUP By `lig`, `bt` Having max(steps) order by `steps` DESC");
