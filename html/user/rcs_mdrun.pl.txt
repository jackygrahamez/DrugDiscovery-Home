...
<job_desc>
    ...
    <task>
        ...
        <weight>1</weight>
        <command_line> a -y -tzip out.7z *.log *.tpr *.gro *.mdp *.edr *.cpt job.xml *.top *.xtc</command_line>
    </task>
</job_desc>
";
...
$mv_to_tmp="mv ".$results."/rcs_*".$rcs_target."*.7z ".$tmp;
print $mv_to_tmp . "\n";
system($mv_to_tmp);

#All Priority groups in sample_results directory
@files = `ls /home/boincadm/projects/DrugDiscovery/sample_results/$rcs_target\/*rcs_*$rcs_target\*.7z`;
sleep(5);
...
#change to new directory
  ...
  $ARG="php update_docking_results.php ".$results."/scores_".$time.".txt";
  print $ARG."\n";
  system($ARG);
...
print "ts: ".$ts."\n";
$wu_name="nsteps_".$nsteps."_int_".$start."_-_".$end."_bt_rcs_".$bt."_lig_".$lig."_ts_".$ts;
...
#Acpypi preps the Ligand file for GROMACS
        $ARG5="/usr/local/bin/acpypi -d -n 0 -i Ligand.pdb -s 120";
        print $ARG5 . "\n";
	system($ARG5);
...
#Prep the Protein2.pb useing pdb2gmx
        $ARG8="pdb2gmx -ff amber99sb -f ".$top_pdb." -o Protein2.pdb -p Protein.top -water spce -ignh";
        print $ARG8 . "\n";
        system($ARG8);
...
#copy the md.tpr used by mdrun workunit. Copy to the download directory using the dir_hier_path which sets the proper fanout directory
        ...
        $ARG1="cp ".$new_score."/md_job.xml \`bin/dir_hier_path Complex".$wu_name.".txt\`";
        print $ARG1 . "\n";
        system($ARG1);
...
$ARG2=$PROJECT."/bin/create_work -appname mdrun -wu_name ".$wu_name." -wu_template ".$PROJECT."/templates/mdrun_wu -result_template templates/mdrun_result --rsc_fpops_est ".$rsc_fpops_est." --rsc_fpops_bound ".$rsc_fpops_bound." --delay_bound ".$delay_bound." md_".$wu_name.".tpr md_".$wu_name.".mdp md_job_".$wu_name.".txt Complex".$wu_name.".txt";
